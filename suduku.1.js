...
rows="3 6 7 5 3 5 6 2 9 1 2 7 0 9 3 6 0 6 2 6 1 8 7 9 2 0 2 3 7 5 9 2 2 8 9 7 3 6 1 2 9 3 1 9 4 7 8 4 5 0 3 6 1 0 6 3 2 0 6 1 5 5 4 7 6 5 6 9 3 7 4 5 2 5 4 7 4 4 3 0 7";
...
function isValidij(board,i,j,val){
   if( board[i].find(item=> item==val) ) return false;
   if( board.find(row=> row.find((item,index )=> index==j&&item==val )) )
   {
       return false;
   };

    if(i<3&&j<3){
       if( board.find((row,rowindex)=> row.find((item,colIndex )=> rowindex<3&&colIndex<3 && item==val )) ) return false;
    }else if(i<3&&j<6){
        if( board.find((row,rowindex)=> row.find((item,colIndex )=> rowindex<3&&colIndex>=3&&colIndex<6 && item==val )) ) return false;
        
    }else if(i<3){
        if( board.find((row,rowindex)=> row.find((item,colIndex )=> rowindex<3 &&colIndex>=6 &&  item==val )) ) return false;
    
    }
    else if(i<6&&j<3){
        if( board.find((row,rowindex)=> row.find((item,colIndex )=>rowindex>=3&& rowindex<6&&colIndex<3 && item==val )) ) return false;
    
    }
    else if(i<6&&j<6){
        if( board.find((row,rowindex)=> row.find((item,colIndex )=> rowindex>=3&& rowindex<6&&colIndex>=3&&colIndex<6 && item==val )) ) return false;
    
    }else if(i<6){
        if( board.find((row,rowindex)=> row.find((item,colIndex )=> rowindex>=3&& rowindex<6&& colIndex>=6 && item==val )) ) return false;
    
    } else if(j<3){
        if( board.find((row,rowindex)=> row.find((item,colIndex )=> rowindex>=6&& colIndex<3 && item==val )) ) return false;
    }
    else if(j<6){
        if( board.find((row,rowindex)=> row.find((item,colIndex )=> rowindex>=6&& colIndex>=3 && colIndex<6 && item==val )) ) return false;
    }  else{
        if( board.find((row,rowindex)=> row.find((item,colIndex )=> rowindex>=6&& colIndex>=6&& item==val )) ) return false;
    }
   return true;
}
function sudokuSimple(board,i,j){
    if(i>=9){console.log(board);return true};
    if(board[i][j]=='0'){
        for (let val=1;val<=9;++val){
            if( isValidij(board,i,j,val+'')){
                board[i][j]=val+'';
                let newi=i;
                let newj=j;
                if(j==8){
                ...
                if(sudokuSimple(board,newi,newj))
                    return true;
            }
        }board[i][j]='0';
    }else{
        ...
        return sudokuSimple(board,newi,newj);
    }
}

console.log(board,"\n","res=",sudokuSimple(board,0,0));
