...
var findAllConcatenatedWordsInADict = function(words) {
    ...
    words.forEach(word=>{
        if(word.length){
            ...
            if(wMap.has(word[0])){
                wMap.get(word[0]).add(word);
            }else{
              wMap.set(word[0],new Set());
                wMap.get(word[0]).add(word);
            }
        }
    ...
    let res=new Set();
    let jmin=0;
    for(let i=jmin;i<wordslength;++i){
        let iItem=words[i];
        for(let j=jmin;j<wordslength;++j){
            let item=words[i]+words[j];
            if(item.length>maxLength)
                break;
            //console.log(item);
            if(wMap.has(item[0])){
                let mapItems=wMap.get(item[0]);
                if(mapItems.has(item)){
                    
                    res.add(item);
                }else{
                    for(let mapItem of mapItems){
                        if(mapItem.startsWith(item)){
                            words.push(mapItem);
                            wMap.get(mapItem[0]).add(item);
                            wordslength++;
                        }
                    }
        ...
        //if(iItem&&iItem.length>0)
         //   if(wMap.get(iItem[0])){
          //      wMap.get(iItem[0]).delete(iItem);
           // }
         //words.shift();
        //console.log(i,words[i],words[i].charAt(0))
       // let temp=words[i];
           
       // if(temp)
       //{  wMap.get(temp.charAt(0)).delete(words[i]);
      //   words.shift();}
      // words.shift();
        //jmin++;
    }
