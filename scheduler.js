var leastInterval = function(tasks, n) {
    ...
    tasks.forEach(item=>{
        if(map.has(item)){
            let counter=map.get(item)+1;
            topCounter=(counter < topCounter)?topCounter : counter;
            map.set(item,counter);
        }else{
             map.set(item,1);
        }
    });
    console.log(map,topCounter);
    let set=new Set();
    ...
    let res=[];
    let adderConstant=(set.size>n)?set.size-1:n;
    arr.forEach(item=>{
        ...
        for(let i=0;i<item[1];){
            if(!res[refPosition]){
                res[refPosition]=item[0];
                refPosition=refPosition+adderConstant+1;
                i++;
            }else{
